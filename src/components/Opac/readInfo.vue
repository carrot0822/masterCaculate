      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column align="center" label="操作" fixed="right" width="200">
            ...
            <template slot-scope="scope">
              <span class="revise" @click="dealBtn(scope.$index, scope.row)">重置密码</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
import { readerInfoInt } from "@request/api/opac/readInfo.js";
export default {
  data() {
    return {
      /*------ 非弹框数据 ------*/
  ...
  methods: {
    ...
    searchBtn() {
      ...
      if (value) {
        switch (value) {
          ...
          case 2:
            this.clearValue(this.searchForm);
            this.searchForm.idCard = this.searchInput.search;
            break;
        }
      } else {
        this.clearValue(this.searchForm);
    ...
    _search(obj) {
      let data = obj;
      readerInfoInt.search(data).then(res => {
    ...
    _deal(obj) {
      let data = obj;
      readerInfoInt.deal(data).then(res => {
        if (res.data.state == true) {
