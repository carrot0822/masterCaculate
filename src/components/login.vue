    ...
    <div id="backgroundImg">
      <div style="position: absolute;top: 100px;left:680px">
        ...
        <div class="divHeight">
          ...
          />
          <img
            src="../base/img/currency/yanjing.png"
            id="show"
            style="width: 25px;height: 25px;position: absolute;top: 33px;left: 270px"
            @click="isShowCheck"
          />
        </div>
        <div class="divHeight" id="yzm">
          <input
            ...
            class="inputHeight"
            maxlength="4"
            style="position: absolute;left: 0;width: 150px"
            v-model="form.yzm"
          />
          <span>
            <img src id="imgYzm" @click="imgClick" />
          </span>
        </div>
...
import { loginInter, login, logoInter } from "../request/api/base.js";
...
export default {
  ...
  methods: {
    ...
    _getLogo() {
      axios.get(logoInter).then(res => {
        if (res.data.state) {
          if (res.data.row == null) {
            let obj = {};
            obj.manageSystemLogoAddress = this.logoNomalUrl;
            obj.manageSystemName = this.nomalName;
            this.$store.dispatch("logo", obj);
    ...
    submit() {
      // 提交1.token的获取存储到Vuex和一个地方 2.路由信息的获取存储 3. 菜单信息 动态路由的生成
      console.log("???执行了没");
      if ($("#name").val() && $("#pwd").val()) {
        $("#msg").html("");
        console.log("登陆");
