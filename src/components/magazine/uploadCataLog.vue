<template>
  <div class="uploadBook" id="bookUpload">
    <div class="title mb_30">
      <div class="sonTitle">
        <span class="titleName">期刊导入</span>
      </div>
    </div>
    <!-- 附件上传 -->
    ...
    <div v-if="active==3" class="stepThree">
      <div class="end">
        ...
        <p>
          <el-button type="primary" @click="backOne">继续导入期刊</el-button>
          <el-button type="primary" @click="backBtn">返回期刊编目</el-button>
        </p>
      </div>
    </div>
  </div>
...
import { uploadInt, indexloadInt } from "@request/api/base.js";
...
export default {
  data() {
    return {
      ...
      fileContent: [],
      backUrl: "", // 返回的url
      twiceList: [], // 二次选择多选框 准确的说是 存储传递数据的数组
      twiceUrl: "",
      active: 0,
      value: "",
      optionData: [], // 下拉框数据
  ...
  methods: {
    ...
    // 离开前需要提示用户 单独的路由守卫
    backBtn() {
      this.$router.push({ path: "/magazineLog" });
    },
    firstStep() {
      let obj = {};
    ...
    searchApi(value) {
      ...
      this.optionData = []
      this.twiceList = []
      axios.post(indexloadInt.getFields, value).then(res => {
    ...
    submitApi(value) {
      ...
      this.next()
      axios.post(indexloadInt.add, value).then(res => {
        if (res.data.state) {
